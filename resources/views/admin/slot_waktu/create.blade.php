@extends('layouts.admin', ['title' => 'Tambah Slot Waktu'])

@section('breadcrumb')
  <a href="{{ route('admin.dasbor_admin') }}" class="hover:underline">Dasbor</a> /
  <a href="{{ route('admin.slot-waktu.index') }}" class="hover:underline">Slot Waktu</a> / Tambah
@endsection
...
<div class="mb-6">
  <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
    <div>
      <h2 class="text-xl font-semibold">Tambah Slot Waktu</h2>
      <p class="text-sm text-gray-600">Buat jadwal slot pemeriksaan baru.</p>
    </div>
...
<form action="{{ route('admin.slot-waktu.store') }}" method="POST" class="bg-white rounded-2xl ring-1 ring-gray-100 p-6 space-y-5">
  @csrf
  ...
  @if(\Illuminate\Support\Facades\Schema::hasColumn('slot_waktu','jenis_tes_id'))
    <div>
      ...
      <select name="jenis_tes_id" class="mt-1 w-full rounded-xl border-gray-300" required>
        ...
        @foreach($jenis as $j)
          <option value="{{ $j->id }}" @selected(old('jenis_tes_id')==$j->id)>{{ $j->nama }}</option>
        @endforeach
      </select>
  ...
  <div class="grid sm:grid-cols-2 gap-4">
    <div>
      ...
      <input type="date" name="tanggal" value="{{ old('tanggal') }}" class="mt-1 w-full rounded-xl border-gray-300" required>
    ...
    @if(\Illuminate\Support\Facades\Schema::hasColumn('slot_waktu','mulai'))
      <div>
        ...
        <input type="time" name="mulai" value="{{ old('mulai') }}" class="mt-1 w-full rounded-xl border-gray-300" required>
    ...
    @if(\Illuminate\Support\Facades\Schema::hasColumn('slot_waktu','selesai'))
      <div>
        ...
        <input type="time" name="selesai" value="{{ old('selesai') }}" class="mt-1 w-full rounded-xl border-gray-300" required>
    ...
    <div>
      ...
      <input type="number" name="kuota" min="1" value="{{ old('kuota', 20) }}" class="mt-1 w-full rounded-xl border-gray-300" required>
      @error('kuota') <p class="text-sm text-rose-600 mt-1">{{ $message }}</p> @enderror
    </div>

    <div>
      <label class="block text-sm text-gray-700">Terpesan (opsional)</label>
      <input type="number" name="terpesan" min="0" value="{{ old('terpesan', 0) }}" class="mt-1 w-full rounded-xl border-gray-300">
